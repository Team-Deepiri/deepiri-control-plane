...
| G10 | **Low-Medium** | Platform | `platform-services/shared/deepiri-prismpipe/` submodule dir is empty while docs/CI reference it; top-level `deepiri-prismpipe` is the real repo. Wiring is ambiguous. |
...
6. **Retire or wire dead schema (G9/G10)**: drop unused LIS models in the next major Prisma migration (or annotate them "reserved"); point the empty prismpipe submodule at the real repo or delete the path.
